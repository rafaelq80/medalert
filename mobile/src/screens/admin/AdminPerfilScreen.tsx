    ...
    <View style={{ flex: 1 }}>
      ...
      <ScrollView style={styles.screen} contentContainerStyle={{ paddingHorizontal: sp.xl, paddingTop: sp.xl, paddingBottom: 40 }}>
        ...
        <View style={styles.avatarSection}>
          <UserBadge nome={user?.nome ?? 'A'} size={72} />
          <Text style={styles.name}>{user?.nome}</Text>
          <Text style={styles.email}>{user?.email}</Text>
          <View style={styles.tipoBadge}><Text style={styles.tipoBadgeText}>Administrador</Text></View>
        </View>
        ...
        <View style={[common.card, { marginBottom: sp.md }]}>
          ...
          {editing ? (
            ...
            <View style={{ gap: 2 }}>
              <View style={styles.infoRow}><Text style={common.textSecondary}>Telefone</Text><Text style={common.textPrimary}>{user?.telefone || '—'}</Text></View>
            </View>
          )}
        </View>
        ...
        <View style={[common.card, { marginBottom: sp.md }]}>
          ...
          <Text style={[common.textSecondary, { marginTop: sp.md, fontSize: 12 }]}>
            Para ajustar o tamanho do texto, use as configurações de acessibilidade do dispositivo.
          </Text>
        </View>
...
const styles = StyleSheet.create(theme => ({
  ...
  tipoBadge: { marginTop: 6, backgroundColor: '#c62828', borderRadius: 9999, paddingHorizontal: 10, paddingVertical: 3 },
  tipoBadgeText: { fontSize: 12, fontWeight: '600', color: '#ffffff' },
  ...
  saveBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingVertical: 8, paddingHorizontal: 14, borderRadius: 8, backgroundColor: theme.primaryContainer },
